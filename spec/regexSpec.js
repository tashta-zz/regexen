describe("your regexes", function(){
  ...
  it("should match ip addresses", function(){
    ...
    var regex = /(([0-1]{0,1}[0-9]{1,2}|2[0-4][0-9]|25[0-5])\.){3}([0-1]{0,1}[0-9]{1,2}|2[0-4][0-9]|25[0-5])/;
  ...
  it("should filter usernames", function(){
    ...
    var regex = /^[A-Za-z]([A-Za-z]|\-|\s|\_){3,19}$/;
  ...
  xit("should extract text", function(){

    var extractText = function(str){
      // var matches = /([^>]+?(?=<))|([a-z]|\s)+$/g.exec(str);
      // var matches = /[^>]+(?=<)|(\w|\s)+$/g.exec(str);
      var matches = /(?<=\>)(\w|\s)+$/g.exec(str);
      return matches;
  ...
  it("should detect nearby words", function(){
    ...
    var containsNearbyWords = function(haystack, needle){
      var reg = new RegExp(needle+"((?=\\s)[A-Za-z]+(?=\\s)){0,3}"+needle);
